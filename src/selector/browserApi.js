...
function adaptTabs(tabs) {
	...
	t.captureVisibleTab = function (windowId, options, callback) {
			...
			if (isBrowser) {
				try {
					...
					if (windowId === null || typeof windowId === "undefined") {
						tabs.captureVisibleTab(options).then((res) => callback(res)).catch(() => callback(undefined));
					} else {
						tabs.captureVisibleTab(windowId, options).then((res) => callback(res)).catch(() => callback(undefined));
					}
				} catch (err) {
					try {
		...
		if (isBrowser) {
			return windowId === null || typeof windowId === "undefined" ? tabs.captureVisibleTab(options) : tabs.captureVisibleTab(windowId, options);
		}
